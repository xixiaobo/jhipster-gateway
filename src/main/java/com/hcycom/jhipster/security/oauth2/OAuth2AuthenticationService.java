package com.hcycom.jhipster.security.oauth2;
...
public class OAuth2AuthenticationService {
	...
	/**
	 * Number of seconds to cache refresh token grants so we don't have to
	...
	public OAuth2AuthenticationService(OAuth2TokenEndpointClient authorizationClient, OAuth2CookieHelper cookieHelper) {
		...
		recentlyRefreshed = CacheBuilder.newBuilder().expireAfterWrite(REFRESH_TOKEN_CACHE_SECS, TimeUnit.SECONDS)
				.build();
	}
	 ...
	 * @return the OAuth2AccessToken as a ResponseEntity. Will return OK (200),
	 *         if successful. If the UAA cannot authenticate the user, the
	 *         status code returned by UAA will be returned.
	 */
	...
	public OAuth2AccessToken getAuthenticate(HttpServletRequest request, HttpServletResponse response,
			Map<String, String> params) {
		try {
			String username = params.get("username");
			String password = params.get("password");
